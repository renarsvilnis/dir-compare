...
var testSync = function (test, testDirPath, saveReport, runOptions) {
...
var testAsync = function (test, testDirPath, saveReport, runOptions) {
    process.chdir(testDirPath);
    var path1, path2;
    if (test.withRelativePath) {
        path1 = test.path1;
...
function testCommandLineInternal(test, testDirPath, async, saveReport, runOptions) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        var command = util.format("node %s %s %s %s %s", dircompareJs, test.commandLineOptions, async ? '--async' : '', path1, path2);
...
var testCommandLine = function (test, testDirPath, saveReport, runOptions) {
    return Promise.all([
...
function executeTests(testDirPath, runOptions) {
    ...
    Promise.resolve(tests_1.getTests(testDirPath)).then(function (tests) {
        // Run sync tests
        ...
        tests_1.getTests(testDirPath).filter(function (test) { return !test.onlyCommandLine; })
        ...
        tests_1.getTests(testDirPath).filter(function (test) { return !test.onlyLibrary; })
            .filter(function (test) { return test.nodeVersionSupport === undefined || semver.satisfies(process.version, test.nodeVersionSupport); })
            .filter(function (test) { return runOptions.singleTestName ? test.name === runOptions.singleTestName : true; })
            .forEach(function (test) {
