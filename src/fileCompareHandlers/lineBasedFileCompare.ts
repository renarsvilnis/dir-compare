...
function removeLineEnding(str: string): string {
...
function compareLines(lines1: string[], lines2: string[], options: LineBasedFileCompareSearchOptions): boolean {
...
export interface LineBasedFileCompareSearchOptions {
...
export default function lineBasedFileCompare({ path1, path2, options }: LineBasedFileCompareOptions): Promise<boolean> {
    let fd1: number;
    let fd2: number;
    ...
    return Promise.all([fdQueue.open(path1, 'r'), fdQueue.open(path2, 'r')])
        .then(([fd1, fd2]) => {
            ...
            const compareAsyncInternal = (): Promise<boolean> => {
                return Promise.all([
                    readAsync(fd1, buf1, 0, BUF_SIZE, null),
                    readAsync(fd2, buf2, 0, BUF_SIZE, null),
                ]).then(([readResult1, readResult2]) => {
