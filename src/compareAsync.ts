...
async function buildEntry(
    ...
    const lstat = await fs.promises.lstat(entryAbsolutePath);
    const isSymlink = lstat.isSymbolicLink();

    // Memory usage optimization if user doesn't want to follow symlinks
    const stat = searchOptions.skipSymlinks && isSymlink ? undefined : await fs.promises.stat(entryAbsolutePath);

    // TODO: for now disable the optimization to make types easier
    // const [lstat, stat] = await Promise.all([fs.promises.lstat(entryAbsolutePath), fs.promises.stat(entryAbsolutePath)]);
    // const isSymlink = lstat.isSymbolicLink();
...
async function getEntries(
    ...
    if (stat.isDirectory()) {
        // TODO: look into maybe migrating to fs.opendir(), might increase speed.
        const rawEntries = await fs.promises.readdir(absolutePath);
...
export default async function compareAsyncInternal({
    ...
    while (i1 < entries1LengthCached || i2 < entries2LengthCached) {
        ...
        const onSame = (isSame: boolean): void =>
            onDifference(
                isSame
                ...
                if (searchOptions.compareSize && fileStat1!.size !== fileStat2!.size) {
                    ...
                    const compareFile = async () => {
                        ...
                        const isSame = await searchOptions.compareFile({
                            path1: entry1.absolutePath,
                            // stat1: entry1.stat!,
                            path2: entry2.absolutePath,
                            // entry2.stat!,
