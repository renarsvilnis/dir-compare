...
function run() {
  ...
  i = 1;
  Promise.resolve()
    .then(() => dircompare.compare(path1, path2, { compareSize: true }))
    .then(res => console.log(`res${i++} ` + (JSON.stringify(res) === expected1 ? "OK" : "FAIL")))
    .then(() => dircompare.compare(path1, path2, { compareContent: true }))
    .then(res => console.log(`res${i++} ` + (JSON.stringify(res) === expected2 ? "OK" : "FAIL")))
    .then(() => dircompare.compare(path1, path2, { includeFilter: "*.c", compareContent: true }))
